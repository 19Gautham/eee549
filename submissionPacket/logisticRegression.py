...
def get_data(path, required_labels, train=True):
    ...
    for imgs, label in dataset:
      if label in required_labels.keys():
          flattened = imgs.view(imgs.size(0), -1).numpy()
...
def train_model(train_images, train_labels, solver, tol, c_val, penalty="l2"):

    # https://scikit-learn.org/1.5/modules/generated/sklearn.linear_model.LogisticRegression.html
    model = LogisticRegression(penalty=penalty, tol=tol, solver=solver, max_iter=2000, C=c_val, verbose=1)
    # Train the model
...
def tune_model(train_images, train_labels, val_images, val_labels, penalty="l2"):

    solver_values = ['lbfgs', 'sag', 'saga', 'liblinear', 'newton-cg', 'newton-cholesky']
    tol_values = [1e-3, 0.5*1e-3, 1e-4]
    c_values = [1e-4, 0.0005, 0.001, 0.0025, 0.005, 0.01, 0.1, 1, 10, 100, 1e20]

    best_model = None
    best_accuracy = 0
    best_hyperparams = None

    for solver, tol, c_val in itertools.product(solver_values, tol_values, c_values):
        print(f"Hyperparameters: solver={solver}, tol={tol}, c_val={c_val}")

        try:
            model = train_model(train_images, train_labels, solver, tol, c_val)
            ...
            if val_accuracy > best_accuracy:
                ...
                best_model = model
                best_hyperparams = (solver, tol, c_val)

        except Exception as e:
            print(f"Error with hyperparams: solver={solver}, tol={tol}, c_val={c_val} during training: {e}")

    return best_model, best_hyperparams, best_accuracy

def analyze_best_model(best_model, best_hyperparams, train_images, train_labels, val_images, val_labels, test_images, test_labels):
    print(f"Best Hyperparameters: {best_hyperparams}")

    solver, tol, c_val_opt = best_hyperparams

    evaluate_model(best_model, test_images, test_labels)

    class_accuracies = per_class_accuracy(best_model, test_images, test_labels, classes=[5, 6])
    print("Per-Class Accuracy:", class_accuracies)

    plot_roc_curve(best_model, test_images, test_labels)
    ...
    for c_val in c_values:
        try:
            model = LogisticRegression(penalty="l2", C=c_val, tol=tol, max_iter=2000, solver=solver, verbose=0)
            model.fit(train_images, train_labels)

            train_accuracy = model.score(train_images, train_labels)
            val_accuracy = model.score(val_images, val_labels)

            train_accuracies.append(train_accuracy)
            val_accuracies.append(val_accuracy)
            test_accuracies.append(evaluate_model(model, test_images, test_labels))
    ...
    plot_hyperparameter_tuning(c_values, train_accuracies, val_accuracies, test_accuracies)

import matplotlib.pyplot as plt

def per_class_accuracy(model, test_images, test_labels, classes):
    predictions = model.predict(test_images)
    class_accuracies = {}
...
def plot_roc_curve(model, test_images, test_labels):
    probs = model.predict_proba(test_images)[:, 1]  # Probabilities for positive class
    fpr, tpr, _ = roc_curve(test_labels, probs, pos_label=6)  # Assuming class '6' is the positive class
...
def plot_hyperparameter_tuning(c_values, train_accuracies, val_accuracies, test_accuracies):
    plt.figure()
    ...
    plt.ylabel('Accuracy')
    plt.title('Train/Validation Accuracy vs. C')
    plt.xscale('log')
    plt.legend()
    plt.show()
...
def analyzer(best_hyperparams, train_images, train_labels, val_images, val_labels, test_images, test_labels):
    print(f"Best Hyperparameters: {best_hyperparams}")
...
def run_best_model():
    ...
    # let's optimize over a set of parameters
    ...
    hyperparams = ('lbfgs', 0.001, 0.0005)

    model = train_model(normalized_train_data, train_labels, *hyperparams)
...
def train_and_evaluate():
    ...
    # model, hyperparams, accuracy = tune_model(normalized_train_data, train_labels, normalized_val_data, val_labels)
    # evaluate_model(model, normalized_test_data, test_labels)
